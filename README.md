...
- **USBシリアルへのパススルー**  
  UART1で受信したデータをUSBシリアル経由でPCにもそのまま出力します。
- **カーソル点滅**  
...
3. 起動後、外部デバイスのUART1出力が液晶に表示され、同時にUSBシリアル経由でPCにも出力されます。
4. Arduino IDE のシリアルモニター等を開き、`kUsbSerialBaudRate` と同じボーレートで受信内容を確認できます。
